...
export default function VersionButton() {
    ...
    const implementedVersions = versionData.versions.filter((v) => v.status === 'current');
        ...
        <>
            ...
            {showModal && selectedVersion && (
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 px-4">
                    <div className="bg-zinc-900 border border-zinc-800 rounded-2xl max-w-4xl w-full h-[600px] flex flex-col">
                        {/* Header */}
                        ...
                        <div className="flex flex-1 overflow-hidden min-h-0">
                            ...
                            <div className="w-64 border-r border-zinc-800 flex flex-col flex-shrink-0">
                                {/* Implemented Versions - Scrollable */}
                                    ...
                                    <div className="space-y-2 max-h-[180px] overflow-y-auto pr-2">
                                        {implementedVersions.map((version) => (
                                            <button
                                                ...
                                                className={`w-full text-left px-4 py-3 rounded-lg transition-colors ${selectedVersion.version === version.version && selectedVersion.status === 'current'
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-zinc-500">
                                            {selectedVersion.status === 'current' ? 'Released' : 'Expected'}: {selectedVersion.releaseDate}
                                        </p>
                                    </div>
                                    ...
                                    <div>
                                        <h5 className="text-sm font-semibold text-zinc-400 uppercase tracking-wide mb-3">
                                            {selectedVersion.status === 'current' ? "What's Included" : 'Planned Features'}
                                        </h5>
                                        <div className="space-y-3">
                                            {selectedVersion.features.map((feature, idx) => (
                                                <div key={idx} className="flex items-start gap-3">
                                                    ...
                                                    <div>
                                                        <p className={`text-sm font-medium ${selectedVersion.status === 'current' ? 'text-white' : 'text-zinc-300'}`}>
                                                            {feature.title}
                                                        </p>
                                                        <p className={`text-xs ${selectedVersion.status === 'current' ? 'text-zinc-500' : 'text-zinc-600'}`}>
                                                            {feature.description}
                                                        </p>
                                                    </div>
                                                </div>
